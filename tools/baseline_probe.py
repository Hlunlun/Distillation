...
def probe_model(model, preprocess, device, args) -> dict:
    result: dict = {}
    chexpert_ok = (
...
def append_baseline_to_experiments_md(rows: list[dict], run_dir: Path) -> None:
    ...
    for r in rows:
        ...
        lines.append(
            ...
            f"| {_fmt(cm_d.get('macro_auroc'))} | {_fmt(cm_d.get('macro_f1'))} | {_fmt(cm_d.get('macro_recall'))} |"
        )
...
def main():
    ...
    for spec in models_to_run:
        ...
        print(
            ...
            f"{_fmt(cm_d.get('macro_auroc')):>9} {_fmt(cm_d.get('macro_f1')):>6} {_fmt(cm_d.get('macro_recall')):>7}"
        )
    ...
    if loaded_encoders and args.attn_samples > 0:
